    ...
    <div className='hero-section relative border-b-2 overflow-hidden'>
      <div className="bg-[#fff] md:h-auto xl:h-[700px] w-full xl:relative overflow-hidden">
        ...
        <div className="hero-flex flex absolute top-0 left-0 w-full h-full md:overflow-hidden ">
          <div className="bx1 flex-[0.6] h-full  mt-10">
            <div className={`heading text-left max-sm:ml-10 md:ml-16 lg:ml-40 xl:ml-44 md:mt-40 mt-48 transition-transform duration-1000 ${isAnimating ? 'translate-y-[-50px] opacity-0' : 'translate-y-0 opacity-100'}`}>
              ...
              <p className='font-lato hidden lg:block text-[#9b9b9b] xl:text-xl p-8 text-left '>{slides[currentSlide].text}</p>
            </div>
            <button className='bg-[#b0ebdf] md:ml-16 ml-10 mt-6 hover:scale-110 xl:mt-10 xl:ml-44 cursor-pointer p-3 xl:p-4 rounded-full text-lg xl:text-xl tracking-wider font-montserrat font-medium'>Book Now</button>
          </div>

          {/* Large Device Image */}
          <div className="bx2 absolute hidden sm:flex flex-[0.4] h-full ">
            <img
              src={slides[currentSlide].image}
              alt=""
