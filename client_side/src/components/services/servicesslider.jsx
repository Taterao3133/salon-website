...
const ServicesSlider = () => {
  const services = [
    ...
    { id: 3, icon: Hairstyle, title: 'Hair stylinh' },
     ...
     { id: 5, icon: Pedicure, title: 'Pedicure service' },
    { id: 6, icon: Tanning, title: 'Tan removing service' },
    { id: 7, icon: Waxing, title: 'Waxination service' },
    { id: 8, icon: Hairdryer, title: 'Hair-dryer service' },
    { id: 9, icon: Hairstraightner, title: 'Hair-straightner service' },
    { id: 10, icon: Skincare, title: 'skin-Care service' },
  ...
  return (
    <div className="relative overflow-hidden h-40 w-[1300px] m-auto mt-1">
      <Slider {...settings}>
        {services.map((service) => (
          <ServiceCard key={service.id} image={service.icon} title={service.title} />
        ))}
      </Slider>
    </div>
  );
